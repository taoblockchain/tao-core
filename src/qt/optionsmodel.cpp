...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTaoAmount"))
        settings.setValue("nAnonymizeTaoAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeTaoAmount = settings.value("nAnonymizeTaoAmount").toLongLong();
    ...
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-alphasendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTaoAmount"))
        SoftSetArg("-anonymizetaoamount", settings.value("nAnonymizeTaoAmount").toString().toStdString());
}
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTaoAmount:
            return QVariant(nAnonymizeTaoAmount);
        case UseBlackTheme:
