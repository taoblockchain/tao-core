...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            if (IsEncapsulatedAddress(strTemp))
            {
                strValue = value.toString().toStdString();
                wallet->UpdateEncapsulatedAddress(strTemp, strValue, false);
            } else
            {
                wallet->SetAddressBookName(CTaoAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            std::string sTemp = value.toString().toStdString();
            if (IsEncapsulatedAddress(sTemp))
            {
                printf("TODO\n");
            ...
            if(CTaoAddress(rec->address.toStdString()) == CTaoAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CTaoAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CTaoAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CTaoAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        if (strAddress.length() > 75)
        {
            ...
            // -- Check for duplicate addresses
            {
                ...
                wallet->AddEncapsulatedAddress(sxAddr);
            }
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CTaoAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CTaoAddress(strAddress).Get(), strLabel);
            }
        }
    }
    else if(type == Receive)
    {
        ...
        if (addressType == AT_Encapsulated)
        {
            CEncapsulatedAddress newEncapsulatedAddr;
            std::string sError;
            if (!wallet->NewEncapsulatedAddress(sError, strLabel, newEncapsulatedAddr)
                || !wallet->AddEncapsulatedAddress(newEncapsulatedAddr))
            {
                editStatus = KEY_GENERATION_FAILURE;
                return QString();
            }
            strAddress = newEncapsulatedAddr.Encoded();
        } else
        {
            CPubKey newKey;
            ...
            strAddress = CTaoAddress(newKey.GetID()).ToString();
...
{
    {
        ...
        {
            CTaoAddress address_parsed(sAddr);
