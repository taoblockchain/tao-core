    ...
    filter(0)
{
    ...
    if(fLiteMode){
        ...
        if(fAlphaNode){
            ...
            if(!fEnableDarksend){
                ui->toggleDarksend->setText(tr("Start AlphaSend"));
            } else {
                ui->toggleDarksend->setText(tr("Stop AlphaSend"));
            }
...
{
    if(!fLiteMode && !fAlphaNode) disconnect(timer, SIGNAL(timeout()), this, SLOT(darkSendStatus()));
    delete ui;
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTransferAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTaoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(nMaxToAnonymize > nAnonymizeTaoAmount*COIN) nMaxToAnonymize = nAnonymizeTaoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTaoAmount * COIN) {
...
void OverviewPage::darkSendStatus()
{
    ...
    QString s = tr("Last AlphaSend message:\n") + strStatus;

    if(s != ui->darksendStatus->text())
        LogPrintf("Last AlphaSend message: %s\n", strStatus.toStdString());
...
void OverviewPage::darksendReset(){
    ...
    QMessageBox::warning(this, tr("AlphaSend"),
        tr("AlphaSend was successfully reset."),
...
void OverviewPage::toggleDarksend(){
    ...
    if(hasMixed.isEmpty()){
        QMessageBox::information(this, tr("AlphaSend"),
                tr("If you don't want to see internal AlphaSend fees/transactions select \"Received By\" as Type on the \"Transactions\" tab."),
                QMessageBox::Ok, QMessageBox::Ok);
    ...
    if(!fEnableDarksend){
        ...
        if(balance < minAmount){
            ...
            QMessageBox::warning(this, tr("AlphaSend"),
                tr("AlphaSend requires at least %1 to use.").arg(strMinAmount),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        ...
        if (walletModel->getEncryptionStatus() == WalletModel::Locked)
        {
            ...
            if(!ctx.isValid())
            {
                ...
                QMessageBox::warning(this, tr("AlphaSend"),
                    tr("Wallet is locked and user declined to unlock. Disabling AlphaSend."),
                    QMessageBox::Ok, QMessageBox::Ok);
                if (fDebug) LogPrintf("Wallet is locked and user declined to unlock. Disabling AlphaSend.\n");
                return;
            }
        }
