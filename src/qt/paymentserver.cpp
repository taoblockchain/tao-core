...
const int TAO_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString TAO_IPC_PREFIX("tao:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(TAO_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(TAO_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(TAO_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start tao: click-to-pay handler");
    else
