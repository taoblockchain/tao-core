...
void DarksendConfig::clickBasic()
{
    ...
    QMessageBox::information(this, tr("AlphaSend Configuration"),
        tr(
            "AlphaSend was successfully set to basic (%1 and 2 rounds). You can change this at any time by opening Tao's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::clickHigh()
{
    ...
    QMessageBox::information(this, tr("AlphaSend Configuration"),
        tr(
            "AlphaSend was successfully set to high (%1 and 8 rounds). You can change this at any time by opening Tao's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::clickMax()
{
    ...
    QMessageBox::information(this, tr("AlphaSend Configuration"),
        tr(
            "AlphaSend was successfully set to maximum (%1 and 16 rounds). You can change this at any time by opening Bitcoin's configuration screen."
        ).arg(strAmount)
    );
...
void DarksendConfig::configure(bool enabled, int coins, int rounds) {
    ...
    settings.setValue("nAnonymizeTaoAmount", coins);

    nDarksendRounds = rounds;
    nAnonymizeTaoAmount = coins;
}
