...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    if (sAddr.length() > 75)
    {
        if (IsEncapsulatedAddress(sAddr))
            return true;
    };

    CTaoAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CTaoAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        if(recipients[0].useInstantX && total > (Params().AlphaNodeCollateral()/2)){
        ...
        if(recipients[0].useInstantX && newTx->GetValueOut() > (Params().AlphaNodeCollateral()/2)){
            return IXTransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    std::map<int, std::string> mapEncapsulatedNarr;

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.typeInd == AddressTableModel::AT_Encapsulated)
            {
                ...
                if (sxAddr.SetEncoded(sAddr))
                {
                    ...
                    if (EncapsulatedSecret(ephem_secret, sxAddr.scan_pubkey, sxAddr.spend_pubkey, secretShared, pkSendTo) != 0)
                    {
                    ...
                    if (fDebug)
                    {
                        printf("Encapsulated send to generated pubkey %" PRIszu": %s\n", pkSendTo.size(), HexStr(pkSendTo).c_str());
                    ...
                    printf("Couldn't parse encapsulated address!\n");
                    return Aborted;
                } // else drop through to normal
            }

            CScript scriptPubKey;
            scriptPubKey.SetDestination(CTaoAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
        ...
        for (it = mapEncapsulatedNarr.begin(); it != mapEncapsulatedNarr.end(); ++it)
        {
            int pos = it->first;
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CTaoAddress(strAddress).Get();
        std::string strLabel = rcp.label.toStdString();
        {
            ...
            if (rcp.typeInd == AddressTableModel::AT_Encapsulated)
            {
                wallet->UpdateEncapsulatedAddress(strAddress, strLabel, true);
            } else
            {
...
{
    if (address.type() == typeid(CEncapsulatedAddress))
    {
        CEncapsulatedAddress sxAddr = boost::get<CEncapsulatedAddress>(address);
    ...
    {
    QString strAddress = QString::fromStdString(CTaoAddress(address).ToString());
...
{
    // Tao context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CTaoAddress(address).ToString())].push_back(out);
    }
}
