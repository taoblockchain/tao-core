        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            if(coinControl->useDarkSend && rounds < nDarksendRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>AlphaSend will be disabled.</b><br><br>If you still want to use AlphaSend, please deselect all non-nonymized inputs first and then check AlphaSend checkbox again."),
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = QString::fromStdString(CTaoAddress(outputAddress).ToString());
