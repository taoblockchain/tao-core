...
CDarkSendRelay::CDarkSendRelay(CTxIn& vinAlphanodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinAlphanode = vinAlphanodeIn;
    vchSig = vchSigIn;
...
std::string CDarkSendRelay::ToString()
{
    ...
    info << "vin: " << vinAlphanode.ToString() <<
...
{
    CAlphanode* pmn = mnodeman.GetAlphanodeByRank(nRank, nBlockHeight, MIN_POOL_PEER_PROTO_VERSION);
