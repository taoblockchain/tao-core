...
// A helper object for signing messages from Alpha Nodes
CDarkSendSigner darkSendSigner;
// The current darksends in progress on the network
std::vector<CDarksendQueue> vecDarksendQueue;
// Keep track of the used Alpha Nodes
std::vector<CTxIn> vecAlphanodesUsed;
// keep track of the scanning errors I've seen
map<uint256, CDarksendBroadcastTx> mapDarksendBroadcastTxes;
// Keep track of the active Alpha Node
CActiveAlphanode activeAlphanode;

// count peers we've requested the list from
int RequestedAlphaNodeList = 0;
...
{
    if(fLiteMode) return; //disable all darksend/Alpha Node related functionality
    if(!IsBlockchainSynced()) return;

    if (strCommand == "dsa") { //AlphaSend Accept Into Pool
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            std::string strError = _("Incompatible version.");
            LogPrintf("dsa -- incompatible version! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, strError);

            return;
        }

        if(!fAlphaNode){
            std::string strError = _("This is not a Alpha Node.");
            LogPrintf("dsa -- not a Alpha Node! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, strError);
        ...
        std::string error = "";
        CAlphanode* pmn = mnodeman.Find(activeAlphanode.vin);
        if(pmn == NULL)
        {
            std::string strError = _("Not in the Alpha Node list.");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, strError);
            return;
        }
        ...
        if(sessionUsers == 0) {
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountAlphanodesAboveProtocol(MIN_POOL_PEER_PROTO_VERSION)/5 > mnodeman.nDsqCount){
                LogPrintf("dsa -- last dsq too recent, must wait. %s \n", pfrom->addr.ToString().c_str());
                std::string strError = _("Last AlphaSend was too recent.");
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, strError);
                return;
            }
        }
        ...
        if(!IsCompatibleWithSession(nDenom, txCollateral, error))
        {
            ...
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, error);
            return;
        } else {
            LogPrintf("dsa -- is compatible, please submit! \n");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_ACCEPTED, error);
            return;
        }
    } else if (strCommand == "dsq") { //AlphaSend Queue
        ...
        CAlphanode* pmn = mnodeman.Find(dsq.vin);
        if(pmn == NULL) return;

        // if the queue is ready, submit if we can
        if(dsq.ready) {
            if(!pSubmittedToAlphanode) return;
            if((CNetAddr)pSubmittedToAlphanode->addr != (CNetAddr)addr){
                LogPrintf("dsq - message doesn't match current Alpha Node - %s != %s\n", pSubmittedToAlphanode->addr.ToString().c_str(), addr.ToString().c_str());
                return;
            }

            if(state == POOL_STATUS_QUEUE){
                LogPrintf("AlphaSend queue is ready - %s\n", addr.ToString().c_str());
            ...
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountAlphanodesAboveProtocol(MIN_POOL_PEER_PROTO_VERSION)/5 > mnodeman.nDsqCount){
                LogPrint("darksend", "dsq -- Alpha Node sending too many dsq messages. %s \n", pmn->addr.ToString().c_str());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("darksend", "dsq - new AlphaSend queue object - %s\n", addr.ToString().c_str());
            vecDarksendQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }

    } else if (strCommand == "dsi") { //AlphaSend vIn
        std::string error = "";
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
        ...
        if(!fAlphaNode){
            LogPrintf("dsi -- not a Alpha Node! \n");
            error = _("This is not a Alpha Node.");
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, error);
        ...
        //check it like a transaction
        {
            ...
            if (nValueIn > Params().DarkSendPoolMax()) {
                LogPrintf("dsi -- more than AlphaSend pool max! %s\n", tx.ToString().c_str());
                error = _("Value more than AlphaSend pool maximum allows.");
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, error);
                return;
            }
        ...
        if(AddEntry(in, nAmount, txCollateral, out, error)){
            ...
            RelayStatus(sessionID, GetState(), GetEntriesCount(), ALPHANODE_RESET);
        } else {
            pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), ALPHANODE_REJECTED, error);
        }
    } else if (strCommand == "dssu") { //AlphaSend status update
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        if(!pSubmittedToAlphanode) return;
        if((CNetAddr)pSubmittedToAlphanode->addr != (CNetAddr)pfrom->addr){
            //LogPrintf("dssu - message doesn't match current Alpha Node - %s != %s\n", pSubmittedToAlphanode->addr.ToString().c_str(), pfrom->addr.ToString().c_str());
            return;
        }
        ...
        if((accepted != 1 && accepted != 0) && sessionID != sessionIDMessage){
            LogPrintf("dssu - message doesn't match current AlphaSend session %d %d\n", sessionID, sessionIDMessage);
            return;
        }

        StatusUpdate(state, entriesCount, accepted, error, sessionIDMessage);

    } else if (strCommand == "dss") { //AlphaSend Sign Final Tx
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        ...
        if(success){
        darkSendPool.Check();
            RelayStatus(darkSendPool.sessionID, darkSendPool.GetState(), darkSendPool.GetEntriesCount(), ALPHANODE_RESET);
        }
    } else if (strCommand == "dsf") { //AlphaSend Final tx
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        if(!pSubmittedToAlphanode) return;
        if((CNetAddr)pSubmittedToAlphanode->addr != (CNetAddr)pfrom->addr){
            //LogPrintf("dsc - message doesn't match current Alpha Node - %s != %s\n", pSubmittedToAlphanode->addr.ToString().c_str(), pfrom->addr.ToString().c_str());
            return;
        }
        int sessionIDMessage;
        CTransaction txNew;
    ...
    } else if (strCommand == "dsc") { //AlphaSend Complete
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
...
bool CDarksendPool::SetCollateralAddress(std::string strAddress){
    ...
    {
        LogPrintf("CDarksendPool::SetCollateralAddress - Invalid AlphaSend collateral address\n");
        return false;
    }
...
//
// Unlock coins after AlphaSend fails or succeeds
//
    ...
    switch(state) {
        ...
        case POOL_STATUS_ACCEPTING_ENTRIES:
            if(entriesCount == 0) {
                ...
                return _("AlphaSend request complete:") + " " + _("Your transaction was accepted into the pool!");
            } else {
                std::string suffix = "";
                if(     showingDarkSendMessage % 70 <= 40) return strprintf(_("Submitted following entries to alphanode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                return strprintf(_("Submitted to alphanode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
            }
        ...
        case POOL_STATUS_ERROR:
            return _("AlphaSend request incomplete:") + " " + lastMessage + " " + _("Will retry...");
        case POOL_STATUS_SUCCESS:
            return _("AlphaSend request complete:") + " " + lastMessage;
        case POOL_STATUS_QUEUE:
            if(     showingDarkSendMessage % 70 <= 30) suffix = ".";
            else if(showingDarkSendMessage % 70 <= 50) suffix = "..";
            else if(showingDarkSendMessage % 70 <= 70) suffix = "...";
            return strprintf(_("Submitted to alphanode, waiting in queue %s"), suffix);;
       default:
...
//
// Check the AlphaSend progress and send client updates if a Alpha Node
//
void CDarksendPool::Check()
{
    if(fAlphaNode) LogPrint("darksend", "CDarksendPool::Check() - entries count %lu\n", entries.size());
    ...
    if(fAlphaNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
    ...
    if((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis()-lastTimeChanged >= 10000) {
        ...
        UnlockCoins();
        SetNull();
        if(fAlphaNode) RelayStatus(sessionID, GetState(), GetEntriesCount(), ALPHANODE_RESET);
    }
}

void CDarksendPool::CheckFinalTransaction()
{
    if (!fAlphaNode) return; // check and relay final tx only on alphanode
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if(!darkSendSigner.SetKey(strAlphaNodePrivKey, strError, key2, pubkey2))
        {
            LogPrintf("CDarksendPool::Check() - ERROR: Invalid Alphanodeprivkey: '%s'\n", strError);
            return;
        }
...
//
// Why bother? AlphaSend uses collateral to ensure abuse to the process is kept to a minimum.
// The submission and signing stages in darksend are completely separate. In the cases where
// a client submits a transaction then refused to sign, there must be a cost. Otherwise they
// would be able to do this over and over again and bring the mixing to a hault.
//
// How does this work? Messages to Alpha Nodes come in via "dsi", these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the Alpha Node
...
//  - AlphaSend is completely free, to pay miners we randomly pay the collateral of users.
    ...
    if(fAlphaNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                Being that AlphaSend has "no fees" we need to have some kind of cost associated
                with using it to stop abuse. Otherwise it could serve as an attack vector and
                allow endless transaction that would bloat Tao and make it unusable. To
...
void CDarksendPool::CheckTimeout(){
    if(!fEnableDarksend && !fAlphaNode) return;
    ...
    // check AlphaSend queue objects for timeouts
    int c = 0;
    ...
    int addLagTime = 0;
    if(!fAlphaNode) addLagTime = 10000; //if we're the client, give the server a few extra seconds before resetting.
...
bool CDarksendPool::IsCollateralValid(const CTransaction& txCollateral){
    ...
    //collateral transactions are required to pay out Params().DarkSendCollateral() as a fee to the miners
    if(nValueIn-nValueOut < Params().DarkSendCollateral()) {
...
//
// Execute a darksend denomination via a Alpha Node.
// This is only ran from clients
//
void CDarksendPool::SendDarksendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount){

    if(fAlphaNode) {
        LogPrintf("CDarksendPool::SendDarksendDenominate() - AlphaSend from a Alpha Node is not supported currently.\n");
        return;
    }

    if(txCollateral == CTransaction()){
        LogPrintf ("CDarksendPool:SendDarksendDenominate() - AlphaSend collateral not set");
        return;
    }
    ...
    // we should already be connected to a Alpha Node
    if(!sessionFoundAlphanode){
        LogPrintf("CDarksendPool::SendDarksendDenominate() - No Alpha Node has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        ...
        LogPrintf("CDarksendPool::SendDarksendDenominate() - Not enough disk space, disabling AlphaSend.\n");
        return;
    }
...
// Incoming message from Alpha Node updating the progress of darksend
...
bool CDarksendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, std::string& error, int newSessionID){
    ...
    if(error.size() > 0) strAutoDenomResult = _("Alpha Node:") + " " + error;
    ...
    if(newState == POOL_STATUS_ACCEPTING_ENTRIES){
        if(newAccepted == 1){
            ...
            sessionFoundAlphanode = true;
            //wait for other users. Alpha Node will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundAlphanode) {
            LogPrintf("CDarksendPool::StatusUpdate - entry not accepted by Alpha Node \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
            DoAutomaticDenominating(); //try another Alpha Node
        }
        if(sessionFoundAlphanode) return true;
    }
...
//
// After we receive the finalized transaction from the Alpha Node, we must
...
bool CDarksendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node){
   ...
   // push all of our signatures to the Alpha Node
...
// AlphaSend transaction was completed (failed or successful)
...
//
// Passively run AlphaSend in the background to anonymize funds based on the given configuration.
//
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if(pindexBest->nHeight - cachedLastSuccess < minBlockSpacing) {
        LogPrintf("CDarksendPool::DoAutomaticDenominating - Last successful AlphaSend action was too recent\n");
        strAutoDenomResult = _("Last successful AlphaSend action was too recent.");
        return false;
    }

    if(mnodeman.size() == 0){
        LogPrint("darksend", "CDarksendPool::DoAutomaticDenominating - No Alpha Nodes detected\n");
        strAutoDenomResult = _("No Alpha Nodes detected.");
        return false;
    }
    ...
    // should not be less than fees in Params().DarkSendCollateral() + few (lets say 5) smallest denoms
    CAmount nLowestDenom = Params().DarkSendCollateral() + darkSendDenominations[darkSendDenominations.size() - 1]*5;

    // if there are no DS collateral inputs yet
    if(!pwalletMain->HasCollateralInputs())
        // should have some additional amount for them
        nLowestDenom += Params().DarkSendCollateral()*4;

    CAmount nBalanceNeedsAnonymized = nAnonymizeTaoAmount*COIN - pwalletMain->GetAnonymizedBalance();

    // if balanceNeedsAnonymized is more than pool max, take the pool max
    if(nBalanceNeedsAnonymized > Params().DarkSendPoolMax()) nBalanceNeedsAnonymized = Params().DarkSendPoolMax();
    ...
    // initial phase, find a Alpha Node
    if(!sessionFoundAlphanode){
        ...
        //if we've used 90% of the Alpha Node list then drop all the oldest first
        int nThreshold = (int)(mnodeman.CountEnabled(MIN_POOL_PEER_PROTO_VERSION) * 0.9);
        LogPrint("darksend", "Checking vecAlphanodesUsed size %d threshold %d\n", (int)vecAlphanodesUsed.size(), nThreshold);
        while((int)vecAlphanodesUsed.size() > nThreshold){
            vecAlphanodesUsed.erase(vecAlphanodesUsed.begin());
            LogPrint("darksend", "  vecAlphanodesUsed size %d threshold %d\n", (int)vecAlphanodesUsed.size(), nThreshold);
        }
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue){
                ...
                bool fUsed = false;
                //don't reuse Alpha Nodes
                BOOST_FOREACH(CTxIn usedVin, vecAlphanodesUsed){
                    if(dsq.vin == usedVin) {
                        fUsed = true;
                        break;
                    }
                ...
                // connect to Alpha Node and submit the queue request
                ...
                if(pnode != NULL)
                {
                    ...
                    if(pmn == NULL)
                    {
                            ...
                            LogPrintf("DoAutomaticDenominating --- dsq vin %s is not in Alpha Node list!", dsq.vin.ToString());
                            continue;
                    }
                    pSubmittedToAlphanode = pmn;
                    vecAlphanodesUsed.push_back(dsq.vin);
                    ...
                    strAutoDenomResult = _("Error connecting to Alpha Node.");
                    dsq.time = 0; //remove node
                    continue;
                }
            }
        ...
        while(i < 10)
        {
            CAlphanode* pmn = mnodeman.FindRandomNotInVec(vecAlphanodesUsed, MIN_POOL_PEER_PROTO_VERSION);
            if(pmn == NULL)
            {
                LogPrintf("DoAutomaticDenominating --- Can't find random alphanode!\n");
                strAutoDenomResult = _("Can't find random Alpha Node.");
                return false;
            }
            ...
            lastTimeChanged = GetTimeMillis();
            LogPrintf("DoAutomaticDenominating -- attempt %d connection to Alpha Node %s\n", i, pmn->addr.ToString().c_str());
            CNode* pnode = ConnectNode((CAddress)pmn->addr, NULL, true);
            if(pnode != NULL){
                pSubmittedToAlphanode = pmn;
                vecAlphanodesUsed.push_back(pmn->vin);
                ...
                vecAlphanodesUsed.push_back(pmn->vin); // postpone MN we wasn't able to connect to
                i++;
                continue;
            }
        }

        strAutoDenomResult = _("No compatible Alpha Node found.");
        return false;
    }
...
bool CDarksendPool::PrepareDarksendDenominate()
{
    ...
    LogPrintf("DoAutomaticDenominating : Running AlphaSend denominate for all rounds. Return '%s'\n", strError);
    if(strError == "") return true;
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    vecSend.push_back(make_pair(scriptCollateral, Params().DarkSendCollateral()*4));
...
bool CDarksendPool::CreateDenominated(int64_t nTotalValue)
{
    ...
    if(!pwalletMain->HasCollateralInputs()) {
        vecSend.push_back(make_pair(scriptCollateral, Params().DarkSendCollateral()*4));
        nValueLeft -= Params().DarkSendCollateral()*4;
    }
    ...
    BOOST_REVERSE_FOREACH(int64_t v, darkSendDenominations){
        ...
        while(nValueLeft - v >= Params().DarkSendCollateral() && nOutputs <= 10) {
            CScript scriptDenom;
            CPubKey vchPubKey;
...
bool CDarksendPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral,  std::string& strReason)
{
    ...
    if((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()){
        ...
        if(sessionUsers >= GetMaxPoolTransactions()) strReason = _("Alpha Node queue is full.");
...
std::string CDarksendPool::GetMessageByID(int messageID) {
    ...
    case ERR_MAXIMUM: return _("Value more than AlphaSend pool maximum allows.");
    case ERR_MN_LIST: return _("Not in the Alpha Node list.");
    case ERR_MODE: return _("Incompatible mode.");
    case ERR_NON_STANDARD_PUBKEY: return _("Non-standard public key detected.");
    case ERR_NOT_A_MN: return _("This is not a Alpha Node.");
    case ERR_QUEUE_FULL: return _("Alpha Node queue is full.");
    case ERR_RECENT: return _("Last AlphaSend was too recent.");
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey){
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash)){
        BOOST_FOREACH(CTxOut out, txVin.vout){
            if(out.nValue == Params().AlphaNodeCollateral()*COIN){
...
bool CDarksendQueue::Sign()
{
    ...
    {
        LogPrintf("CDarksendQueue():Relay - ERROR: Invalid Alphanodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CDarksendQueue::CheckSignature()
{
    ...
    if(pmn != NULL)
    {
        ...
        if(!darkSendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
            return error("CDarksendQueue::CheckSignature() - Got bad Alpha Node address signature %s \n", vin.ToString().c_str());
        }
...
void CDarksendPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    ...
    CNode* pnode = FindNode(pSubmittedToAlphanode->addr);
    if(pnode != NULL) {
...
{
    if(fLiteMode) return; //disable all AlphaSend/Alpha Node related functionality

    // Make this thread recognisable as the wallet flushing thread
    RenameThread("tao-darksend");
    ...
    while (true)
    {
        ...
        if(darkSendPool.IsBlockchainSynced()) {
            ...
            if(c % ALPHANODE_PING_SECONDS == 1) activeAlphanode.ManageStatus();

            if(c % 60 == 0)
            {
                mnodeman.CheckAndRemove();
                mnodeman.ProcessAlphanodeConnections();
                alphanodePayments.CleanPaymentList();
                CleanTransactionLocksList();
            }

            //if(c % ALPHANODES_DUMP_SECONDS == 0) DumpAlphanodes();
