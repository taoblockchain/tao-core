...
const string strMessageMagic = "Tao Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    // Size limits
    if (::GetSerializeSize(*this, SER_NETWORK, PROTOCOL_VERSION) > maxBlockSize)
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if (nSigOps > maxStandardTxSigops)
            return tx.DoS(0,
                          error("AcceptToMemoryPool : too many sigops %s, %d > %d",
                                hash.ToString(), nSigOps, maxStandardTxSigops));
        ...
        if (nSigOps > maxBlockSigops)
            return tx.DoS(0,
                          error("AcceptableInputs : too many sigops %s, %d > %d",
                                hash.ToString(), nSigOps, maxBlockSigops));
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, Params().CoinbaseMaturity() - GetDepthInMainChain());
}
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    if (nHeight <= 100)
    {
        ...
        if (nHeight >= Params().POWRewardBlock())
        {
            ...
            int64_t nSubsidy = Params().BasePOWReward();
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (GetCurrentBlockHeight() >= (Params().POWRewardBlock() + 500))
        nSubsidy = nCoinAge * Params().BaseProofOfStakeReward(GetCurrentBlockHeight()) / 365;
    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = Params().TargetBlockSpacing();  // 10 mins
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64_t _nTargetTimespan = CountBlocks * Params().TargetBlockSpacing();
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, Params().MinStakeConfirmations() - 1, nSpendDepth))
        {
...
// ----------- instantX transaction scanning -----------
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx){
        if (!tx.IsCoinBase()){
            ...
            BOOST_FOREACH(const CTxIn& in, tx.vin){
                if(mapLockedInputs.count(in.prevout)){
                    if(mapLockedInputs[in.prevout] != tx.GetHash()){
                        mapRejectedBlocks.insert(make_pair(GetHash(), GetTime()));
    ...
    bool AlphanodePayments = false;
    bool fIsInitialDownload = IsInitialBlockDownload();

    if(nTime > START_ALPHANODE_PAYMENTS) AlphanodePayments = true;
    if (!fIsInitialDownload)
    {
        if(AlphanodePayments)
        {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    // If we don't already have its previous block, skip alphanode payment step
                    CAmount alphanodePaymentAmount;
                    for (int i = vtx[1].vout.size(); i--> 0; ) {
                        alphanodePaymentAmount = vtx[1].vout[i].nValue;
                        break;
                    }
                    ...
                    CScript payee;
                    CTxIn vin;
                    if(!alphanodePayments.GetBlockPayee(pindexBest->nHeight+1, payee, vin) || payee == CScript()){
                        foundPayee = true; //doesn't require a specific payee
                        foundPaymentAmount = true;
                        foundPaymentAndPayee = true;
                        if(fDebug) { LogPrintf("CheckBlock() : Using non-specific alphanode payments %d\n", pindexBest->nHeight+1); }
                    }

                    for (unsigned int i = 0; i < vtx[1].vout.size(); i++) {
                        if(vtx[1].vout[i].nValue == alphanodePaymentAmount )
                            foundPaymentAmount = true;
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            foundPayee = true;
                        if(vtx[1].vout[i].nValue == alphanodePaymentAmount && vtx[1].vout[i].scriptPubKey == payee)
                            foundPaymentAndPayee = true;
                    }
                    ...
                    if(!foundPaymentAndPayee) {
                        if(fDebug) { LogPrintf("CheckBlock() : Couldn't find alphanode payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, alphanodePaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight+1); }
                        mapRejectedBlocks.insert(make_pair(GetHash(), GetTime()));
                        return DoS(100, error("CheckBlock() : Couldn't find alphanode payment or payee"));
                    } else {
                        LogPrintf("CheckBlock() : Found payment(%d|%d) or payee(%d|%s) nHeight %d. \n", foundPaymentAmount, alphanodePaymentAmount, foundPayee, address2.ToString().c_str(), pindexBest->nHeight+1);
                    }
                } else {
                    if(fDebug) { LogPrintf("CheckBlock() : Skipping alphanode payment check - nHeight %d Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
                }
            } else {
                if(fDebug) { LogPrintf("CheckBlock() : pindex is null, skipping alphanode payment check\n"); }
            }
        } else {
            if(fDebug) { LogPrintf("CheckBlock() : skipping alphanode payment checks\n"); }
        }
    } else {
        if(fDebug) { LogPrintf("CheckBlock() : Is initial download, skipping alphanode payment check %d\n", pindexBest->nHeight+1); }
    }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if(!IsInitialBlockDownload()){
        ...
        // If we're in LiteMode disable darksend features without disabling alphanodes
        if (!fLiteMode && !fImporting && !fReindex && pindexBest->nHeight > Checkpoints::GetTotalBlocksEstimate()){

            if(alphanodePayments.GetBlockPayee(pindexBest->nHeight, payee, vin)){
                //UPDATE ALPHANODE LAST PAID TIME
                CAlphanode* pmn = mnodeman.Find(vin);
                if(pmn != NULL) {
                    pmn->nLastPaid = GetAdjustedTime();
                }

                LogPrintf("ProcessBlock() : Update Alpha Node Last Paid Time - %d\n", pindexBest->nHeight);
            }

            darkSendPool.CheckTimeout();
            darkSendPool.NewBlock();
            alphanodePayments.ProcessBlock(GetHeight()+10);
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
    ...
    case MSG_ALPHANODE_WINNER:
        return mapSeenAlphanodeVotes.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_ALPHANODE_WINNER) {
                    if(mapSeenAlphanodeVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mapSeenAlphanodeVotes[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "tx"|| strCommand == "dstx")
    {
        ...
        if(strCommand == "tx") {
            ...
            if (AlreadyHave(txdb, inv))
                return true;
            //these allow alphanodes to publish a limited amount of free transactions
            ...
            {
                if(!pmn->allowFreeTx){
                    //multiple peers can send us a valid alphanode transaction
                    if(fDebug) LogPrintf("dstx: Alpha Node sending too many transactions %s\n", tx.GetHash().ToString().c_str());
                    return true;
                }
                ...
                if(!darkSendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
                    LogPrintf("dstx: Got bad alphanode address signature %s \n", vin.ToString().c_str());
                    //Misbehaving(pfrom->GetId(), 20);
                    return false;
                }

                LogPrintf("dstx: Got Alpha Node transaction %s\n", tx.GetHash().ToString().c_str());
    ...
    else
    {
        ...
        ProcessMessageAlphanodePayments(pfrom, strCommand, vRecv);
...
int64_t GetAlphanodePayment(int nHeight, int64_t blockValue)
{
