...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CTaoAddress(redeemScript.GetID()).ToString();
...
bool CWallet::Lock()
{
...

    {
        ...
        // -- load encrypted spend_secret of encapsulated addresses
        CEncapsulatedAddress sxAddrTemp;
        std::set<CEncapsulatedAddress>::iterator it;
        for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
        {
            if (it->scan_secret.size() < 32)
                continue; // encapsulated address is not owned
            // -- CEncapsulatedAddress are only sorted on spend_pubkey
            CEncapsulatedAddress &sxAddr = const_cast<CEncapsulatedAddress&>(*it);
            if (fDebug)
                printf("Recrypting stealth key %s\n", sxAddr.Encoded().c_str());

            sxAddrTemp.scan_pubkey = sxAddr.scan_pubkey;
            if (!wdb.ReadEncapsulatedAddress(sxAddrTemp))
            {
...
bool CWallet::Unlock(const SecureString& strWalletPassphrase, bool anonymizeOnly)
{
    ...
    fWalletUnlockAnonymizeOnly = anonymizeOnly;
    UnlockEncapsulatedAddresses(vMasterKey);
...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
...

    {
        ...
        for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
        {
            if (it->scan_secret.size() < 32)
                continue; // encapsulated address is not owned
            // -- CEncapsulatedAddress is only sorted on spend_pubkey
            CEncapsulatedAddress &sxAddr = const_cast<CEncapsulatedAddress&>(*it);
            ...
            pwalletdbEncryption->WriteEncapsulatedAddress(sxAddr);
        };
...
bool CWallet::AddToWalletIfInvolvingMe(const CTransaction& tx, const CBlock* pblock, bool fUpdate)
{
    uint256 hash = tx.GetHash();
    {
        ...
        mapValue_t mapNarr;
        FindEncapsulatedTransactions(tx, mapNarr);
...
// Recursively determine the rounds of a given input (How deep is the AlphaSend chain for a given input)
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s TAO %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !(fAlphaNode && pcoin->vout[i].nValue == Params().AlphaNodeCollateral());
                } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN){
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fAlphaNode) found = pcoin->vout[i].nValue != Params().AlphaNodeCollateral(); // do not use Hot MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
void CWallet::AvailableCoinsForStaking(vector<COutput>& vCoins, unsigned int nSpendTime) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (nDepth < Params().MinStakeConfirmations())
            {
                continue;
            }
            else
            ...
            if (pcoin->nTime + Params().StakeMinAge(GetCurrentBlockHeight()) > nSpendTime)
               continue;
            }
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++){
                ...
                if (pcoin->vout[i].nValue == Params().AlphaNodeCollateral()){

                    //LogPrintf("CWallet::AvailableCoinsForStaking - Found Alpha Node collateral.\n");
                    found = true;
                    break;
                }
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        // alphanode-like input should not be selected by AvailableCoins now anyway
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fAlphaNode && out.tx->vout[out.i].nValue == Params().AlphaNodeCollateral()) continue; //alphanode input
...
{
    return  nInputAmount != 0 && nInputAmount % Params().DarkSendCollateral() == 0 && nInputAmount < Params().DarkSendCollateral() * 5  && nInputAmount > Params().DarkSendCollateral();
}
...
bool CWallet::CreateCollateralTransaction(CTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: AlphaSend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if(nValueIn2 - Params().DarkSendCollateral() - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - Params().DarkSendCollateral(), scriptChange);
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey,
                                ...
                                const std::string& txData, AvailableCoinsType coin_type, bool useIX)
{
    ...
    if ((vecSend.empty() && !fIntelliTx) || nValue < 0)
    {
        strFailReason = _("Transaction amounts must be positive");
        return false;
    }
...

    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if(useIX || fIntelliTx) nFeeRet = max(CENT, nFeeRet);
            while (true)
            {
                wtxNew.vin.clear();
                wtxNew.vout.clear();
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough AlphaSend non-denominated funds for this transaction.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN ) {
                        strFailReason = _(" Unable to locate enough AlphaSend non-denominated funds for this transaction that are not equal 1000 TAO.");
                    } else {
                        strFailReason = _(" Unable to locate enough AlphaSend denominated funds for this transaction.");
                        strFailReason += _(" AlphaSend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-tao-address
                    CScript scriptChange;
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, const std::string& txData)
{
    ...
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePos, strFailReason, coinControl, txData);
    if(!strFailReason.empty())
    {
        LogPrintf("CreateTransaction(): ERROR: %s\n", strFailReason);
        return false;
    }
    // -- narration will be added to mapValue later in FindEncapsulatedTransactions From CommitTransaction
    return rv;
}


bool CWallet::NewEncapsulatedAddress(std::string& sError, std::string& sLabel, CEncapsulatedAddress& sxAddr)
{
        ...
        || GenerateRandomSecret(spend_secret) != 0)
    {
        ...
        printf("Error CWallet::NewEncapsulatedAddress - %s\n", sError.c_str());
        return false;
    };
    ...
    if (fDebug)
    {
        printf("getnewencapsulatedaddress: ");
        printf("scan_pubkey ");
...
bool CWallet::AddEncapsulatedAddress(CEncapsulatedAddress& sxAddr)
{
    LOCK(cs_wallet);
    ...
    if (fOwned)
    {
        ...
        if (IsLocked())
        {
            printf("Error: CWallet::AddEncapsulatedAddress wallet must be unlocked.\n");
    ...
    bool rv = CWalletDB(strWalletFile).WriteEncapsulatedAddress(sxAddr);
...
bool CWallet::UnlockEncapsulatedAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    // -- decrypt spend_secret of encapsulated addresses
    std::set<CEncapsulatedAddress>::iterator it;
    for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
    {
        if (it->scan_secret.size() < 32)
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CTaoAddress addr(ckid);

        EncapsulatedKeyMetaMap::iterator mi = mapEncapsulatedKeyMeta.find(ckid);
        if (mi == mapEncapsulatedKeyMeta.end())
        {
            printf("Error: No metadata found to add secret for %s\n", addr.ToString().c_str());
            continue;
        };

        CEncapsulatedKeyMetadata& sxKeyMeta = mi->second;

        CEncapsulatedAddress sxFind;
        sxFind.scan_pubkey = sxKeyMeta.pkScan.Raw();

        std::set<CEncapsulatedAddress>::iterator si = stealthAddresses.find(sxFind);
        ...
        {
            printf("Encapsulated address has no secret key for %s\n", addr.ToString().c_str());
            continue;
        }
        ...
        if (EncapsulatedSecretSpend(sScan, pkEphem, sSpend, sSpendR) != 0)
        {
            printf("EncapsulatedSecretSpend() failed.\n");
            continue;
        };
        ...
        if (!CWalletDB(strWalletFile).EraseEncapsulatedKeyMeta(ckid))
            printf("EraseEncapsulatedKeyMeta failed for %s\n", addr.ToString().c_str());
    };
    return true;
}

bool CWallet::UpdateEncapsulatedAddress(std::string &addr, std::string &label, bool addIfNotExist)
{
    if (fDebug)
        printf("UpdateEncapsulatedAddress %s\n", addr.c_str());
    ...
    if (it == stealthAddresses.end())
    {
        ...
        {
            printf("UpdateEncapsulatedAddress %s, not in set\n", addr.c_str());
            return false;
        };
    } else
    {
        sxFound = const_cast<CEncapsulatedAddress&>(*it);
        ...
        {
            printf("UpdateEncapsulatedAddress: todo - update owned encapsulated address.\n");
            return false;
        };
    };
    ...
    if (!CWalletDB(strWalletFile).WriteEncapsulatedAddress(sxFound))
    {
        printf("UpdateEncapsulatedAddress(%s) Write to db failed.\n", addr.c_str());
        return false;
    };
...
bool CWallet::CreateEncapsulatedTransaction(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl, const std::string& txData)
{
    ...
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, nChangePos, strFailReason, coinControl, txData);
    if(!strFailReason.empty())
        LogPrintf("CreateEncapsulatedTransaction(): %s\n", strFailReason);
    ...
    if (rv && narr.size() > 0)
    {
        for (unsigned int k = 0; k < wtxNew.vout.size(); ++k)
        {
            ...
            {
                printf("CreateEncapsulatedTransaction(): Error creating narration key.");
                break;
            };
...
string CWallet::SendEncapsulatedMoney(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, const std::string& txData)
{
    CReserveKey reservekey(this);
    ...
    if (IsLocked())
    {
        ...
        LogPrintf("SendEncapsulatedMoney() : %s\n", strError.c_str());
        return strError;
    }
    ...
    if (!CreateEncapsulatedTransaction(scriptPubKey, nValue, P, narr, sNarr, wtxNew, reservekey, nFeeRequired, NULL, txData))
    {
        string strError;
...
bool CWallet::SendEncapsulatedMoneyToDestination(CEncapsulatedAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee, const std::string& txData)
{
    // -- Check amount
    if (nValue <= 0)
    {
    ...
    if (EncapsulatedSecret(ephem_secret, sxAddress.scan_pubkey, sxAddress.spend_pubkey, secretShared, pkSendTo) != 0)
    {
    ...
    if (fDebug)
    {
        LogPrintf("Encapsulated send to generated pubkey %" PRIszu": %s\n", pkSendTo.size(), HexStr(pkSendTo).c_str());
    ...
    if ((sError = SendEncapsulatedMoney(scriptPubKey, nValue, ephem_pubkey, vchNarr, sNarr, wtxNew, fAskFee, txData)) != "")
        return false;
...
bool CWallet::FindEncapsulatedTransactions(const CTransaction& tx, mapValue_t& mapNarr)
{
    if (fDebug)
        LogPrintf("FindEncapsulatedTransactions() tx: %s\n", tx.GetHash().GetHex().c_str());
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
            ...
            || vchEphemPK.size() != 33)
        {
                ...
                && vchEphemPK[1] == 'p')
            {
                ...
                {
                    printf("Warning: FindEncapsulatedTransactions() tx: %s, Could not extract plaintext narration.\n", tx.GetHash().GetHex().c_str());
                };
            }
        ...
        BOOST_FOREACH(const CTxOut& txoutB, tx.vout)
        {
            ...
            for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
            {
                ...
                if (EncapsulatedSecret(sScan, vchEphemPK, it->spend_pubkey, sShared, pkExtracted) != 0)
                {
                    printf("EncapsulatedSecret failed.\n");
                    continue;
                };
                ...
                if (IsLocked())
                {
                    ...
                    CEncapsulatedKeyMetadata lockedSkMeta(cpkEphem, cpkScan);

                    if (!CWalletDB(strWalletFile).WriteEncapsulatedKeyMeta(keyId, lockedSkMeta))
                        printf("WriteEncapsulatedKeyMeta failed for %s\n", coinAddress.ToString().c_str());

                    mapEncapsulatedKeyMeta[keyId] = lockedSkMeta;
                    nFoundEncapsulated++;
                } else
                {
                    ...
                    if (EncapsulatedSharedToSecretSpend(sShared, sSpend, sSpendR) != 0)
                    {
                        printf("EncapsulatedSharedToSecretSpend() failed.\n");
                        continue;
                    };
...
uint64_t CWallet::GetStakeWeight() const
{
    ...
    {
        CTxIndex txindex;
        if (pcoin.first->GetDepthInMainChain() >= Params().CoinbaseMaturity())
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
            ...
            && pcoin.first->GetHash() != txNew.vin[0].prevout.hash)
        {
            ...
            if (nTimeWeight < Params().StakeMinAge(GetCurrentBlockHeight()))
                continue;
    ...
    // Alpha Node Payments
    int payments = 1;
    // start alphanode payments
    bool bAlphaNodePayment = false; // note was false, set true to test

    if ( Params().NetworkID() == CChainParams::TESTNET ){
        if (GetTime() > START_ALPHANODE_PAYMENTS_TESTNET ){
            bAlphaNodePayment = true;
        }
    }else{
        if (GetTime() > START_ALPHANODE_PAYMENTS){
            bAlphaNodePayment = true;
        }
    }
    ...
    if(bAlphaNodePayment) {
        //spork
        if(!alphanodePayments.GetBlockPayee(pindexPrev->nHeight+1, payee, vin)){
            CAlphanode* winningNode = mnodeman.GetCurrentAlphaNode(1);
            if(winningNode){
                payee = GetScriptForDestination(winningNode->pubkey.GetID());
            } else {
                return error("CreateCoinStake: Failed to detect alphanode to pay\n");
            }
        }
    }
    ...
    if(hasPayment){
        ...
        LogPrintf("Alpha Node payment to %s\n", address2.ToString().c_str());
    }

    int64_t blockValue = nCredit;
    int64_t alphanodePayment = GetAlphanodePayment(pindexPrev->nHeight+1, nReward);


    // Set output amount
    if (!hasPayment && txNew.vout.size() == 3) // 2 stake outputs, stake was split, no alphanode payment
    {
        txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
        txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    }
    else if(hasPayment && txNew.vout.size() == 4) // 2 stake outputs, stake was split, plus a alphanode payment
    {
        txNew.vout[payments-1].nValue = alphanodePayment;
        blockValue -= alphanodePayment;
        txNew.vout[1].nValue = (blockValue / 2 / CENT) * CENT;
        txNew.vout[2].nValue = blockValue - txNew.vout[1].nValue;
    }
    else if(!hasPayment && txNew.vout.size() == 2) // only 1 stake output, was not split, no alphanode payment
        txNew.vout[1].nValue = blockValue;
    else if(hasPayment && txNew.vout.size() == 3) // only 1 stake output, was not split, plus a alphanode payment
    {
        txNew.vout[payments-1].nValue = alphanodePayment;
        blockValue -= alphanodePayment;
        txNew.vout[1].nValue = blockValue;
...
{
    mapValue_t mapNarr;
    FindEncapsulatedTransactions(wtxNew, mapNarr);
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, const std::string& txData)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
    ...
    if (!CreateTransaction(scriptPubKey, nValue, sNarr, wtxNew, reservekey, nFeeRequired, NULL, txData))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, const std::string& txData)
{
    ...
    return SendMoney(scriptPubKey, nValue, sNarr, wtxNew, fAskFee, txData);
}
...
string CWallet::PrepareDarksendDenominate(int minRounds, int maxRounds)
{
    ...
    if(darkSendPool.GetState() != POOL_STATUS_ERROR && darkSendPool.GetState() != POOL_STATUS_SUCCESS)
        if(darkSendPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the AlphaSend pool");
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(darkSendPool.sessionDenom, 0.1*COIN, Params().DarkSendPoolMax(), vCoins, vCoins2, nValueIn, minRounds, maxRounds))
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CTaoAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CTaoAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CTaoAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s TAO %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s TAO %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
