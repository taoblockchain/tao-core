...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TAO decode failed");
    }
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("POW difficulty",        GetDifficulty()));
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    nExpectedTime = staking ? (Params().TargetBlockSpacing() * nNetworkWeight / nWeight) : 0;
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Tao is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Tao is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Tao is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Tao is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "        ,...\n"
            "  ],\n"
            "  \"alphanode_payments\" : true|false,         (boolean) true, if alphanode payments are enabled"
            "  \"enforce_alphanode_payments\" : true|false  (boolean) true, if alphanode payments are enforced"
