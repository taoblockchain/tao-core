...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: tao.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: tao-daemon.pid)") + "\n";
    ...
    strUsage += "  -litemode=<n>          " + _("Disable all AlphaSend and Encapsulated Messaging related functionality (0-1, default: 0)") + "\n";
strUsage += "\n" + _("Alpha Node options:") + "\n";
    strUsage += "  -alphanode=<n>            " + _("Enable the client to act as a alphanode (0-1, default: 0)") + "\n";
    strUsage += "  -mnconf=<file>             " + _("Specify alphanode configuration file (default: alphanode.conf)") + "\n";
    strUsage += "  -mnconflock=<n>            " + _("Lock alphanodes from alphanode configuration file (default: 1)") + "\n";
    strUsage += "  -alphanodeprivkey=<n>     " + _("Set the alphanode private key") + "\n";
    strUsage += "  -alphanodeaddr=<n>        " + _("Set external address:port to get to this alphanode (example: address:port)") + "\n";
    strUsage += "  -alphanodeminprotocol=<n> " + _("Ignore alphanodes less than version (example: 61401; default : 0)") + "\n";

    strUsage += "\n" + _("AlphaSend options:") + "\n";
    strUsage += "  -enabledarksend=<n>          " + _("Enable use of automated darksend for funds stored in this wallet (0-1, default: 0)") + "\n";
    strUsage += "  -alphasendrounds=<n>          " + _("Use N separate alphanodes to anonymize funds  (2-8, default: 2)") + "\n";
    strUsage += "  -anonymizetaoamount=<n> " + _("Keep N Tao anonymized (default: 0)") + "\n";
    strUsage += "  -liquidityprovider=<n>       " + _("Provide liquidity to AlphaSend by infrequently mixing coins on a continual basis (0-100, default: 0, 1=very frequent, high fees, 100=very infrequent, low fees)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Process alphanode config
    alphanodeConfig.read(GetAlphanodeConfigFile());
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Tao is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Tao is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Tao version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (mapArgs.count("-alphanodepaymentskey")) // alphanode payments priv key
    {
        if (!alphanodePayments.SetPrivKey(GetArg("-alphanodepaymentskey", "")))
            return InitError(_("Unable to sign alphanode payment winner, wrong key?"));
        if (!sporkManager.SetPrivKey(GetArg("-alphanodepaymentskey", "")))
            return InitError(_("Unable to sign spork message, wrong key?"));
    }

    //ignore alphanodes below protocol version
    nAlphanodeMinProtocol = GetArg("-alphanodeminprotocol", MIN_POOL_PEER_PROTO_VERSION);

    if (fDaemon)
        fprintf(stdout, "Tao server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Tao") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Tao to complete") << "\n";
    ...
    uiInterface.InitMessage(_("Loading alphanode cache..."));

    CAlphanodeDB mndb;
    CAlphanodeDB::ReadResult readResult = mndb.Read(mnodeman);
    if (readResult == CAlphanodeDB::FileError)
        LogPrintf("Missing alphanode cache file - alphacache.dat, will try to recreate\n");
    ...
    fAlphaNode = GetBoolArg("-alphanode", false);
    if(fAlphaNode) {
        LogPrintf("IS DARKSEND MASTER NODE\n");
        strAlphaNodeAddr = GetArg("-alphanodeaddr", "");

        LogPrintf(" addr %s\n", strAlphaNodeAddr.c_str());

        if(!strAlphaNodeAddr.empty()){
            CService addrTest = CService(strAlphaNodeAddr, fNameLookup);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -alphanodeaddr address: " + strAlphaNodeAddr);
            }
        }

        strAlphaNodePrivKey = GetArg("-alphanodeprivkey", "");
        if(!strAlphaNodePrivKey.empty()){
            std::string errorMessage;

            CKey key;
            CPubKey pubkey;

            if(!darkSendSigner.SetKey(strAlphaNodePrivKey, errorMessage, key, pubkey))
            {
                return InitError(_("Invalid alphanodeprivkey. Please see documenation."));
            }

            activeAlphanode.pubKeyAlphanode = pubkey;

        } else {
            return InitError(_("You must specify a alphanodeprivkey in the configuration. Please see documentation for help."));
        }
    ...
    nDarksendRounds = GetArg("-alphasendrounds", 2);
    ...
    nAnonymizeTaoAmount = GetArg("-anonymizetaoamount", 0);
    if(nAnonymizeTaoAmount > 999999) nAnonymizeTaoAmount = 999999;
    if(nAnonymizeTaoAmount < 2) nAnonymizeTaoAmount = 2;
    ...
    //lite mode disables all Alpha Node and AlphaSend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fAlphaNode && fLiteMode){
        return InitError("You can not start a alphanode in litemode");
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nInstantXDepth %d\n", nInstantXDepth);
    LogPrintf("AlphaSend rounds %d\n", nDarksendRounds);
    LogPrintf("Anonymize Tao Amount %d\n", nAnonymizeTaoAmount);

    /* Denominations
       A note about convertability. Within AlphaSend pools, each denomination
    ...
    */
    ...
    if (fAlphaNode)
    {
        LogPrintf("AlphaNode active. Staking disabled.");
    } else {
