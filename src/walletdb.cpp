...
bool CWalletDB::WriteEncapsulatedKeyMeta(const CKeyID& keyId, const CEncapsulatedKeyMetadata& sxKeyMeta)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("sxKeyMeta"), keyId), sxKeyMeta, true);
}

bool CWalletDB::EraseEncapsulatedKeyMeta(const CKeyID& keyId)
{
    nWalletDBUpdated++;
    return Erase(std::make_pair(std::string("sxKeyMeta"), keyId));
}

bool CWalletDB::WriteEncapsulatedAddress(const CEncapsulatedAddress& sxAddr)
{
    nWalletDBUpdated++;
...
bool CWalletDB::ReadEncapsulatedAddress(CEncapsulatedAddress& sxAddr)
{
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CTaoAddress(strAddress).Get()];
        }
        else if (strType == "tx")
        {
            uint256 hash;
        ...
        else if (strType == "sxKeyMeta")
        {
            ...
            CKeyID keyId;
            ssKey >> keyId;
            CEncapsulatedKeyMetadata sxKeyMeta;
            ssValue >> sxKeyMeta;

            pwallet->mapEncapsulatedKeyMeta[keyId] = sxKeyMeta;
        }
