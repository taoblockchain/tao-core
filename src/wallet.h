...
typedef std::map<CKeyID, CEncapsulatedKeyMetadata> EncapsulatedKeyMetaMap;
...
public:
    ...
    bool SelectCoinsAlphanode(CTxIn& vin, int64_t& nValueRet, CScript& pubScript) const;
    ...
    std::set<CEncapsulatedAddress> stealthAddresses;
    EncapsulatedKeyMetaMap mapEncapsulatedKeyMeta;

    int nLastFilteredHeight;

    uint32_t nEncapsulated, nFoundEncapsulated; // for reporting, zero before use
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, int32_t& nChangePos, std::string& strFailReason, const CCoinControl *coinControl=NULL, const std::string& txData="", AvailableCoinsType coin_type=ALL_COINS, bool useIX=false);
    bool CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl *coinControl=NULL, const std::string& txData="");
    ...
    std::string SendMoneyToDestination(const CTxDestination &address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, const std::string& txData="", bool fAskFee=false);
    ...
    // get the AlphaSend chain depth for a given input
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            ...
            if(fAlphaNode && vout[i].nValue == Params().AlphaNodeCollateral()) continue; // do not count MN-like outputs
...
public:
    ...
    //Used with AlphaSend. Will return fees, then denominations, everything else, then very small inputs that aren't fees
    int Priority() const
    {
