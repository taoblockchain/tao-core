...
CCriticalSection cs_alphanodes;
// keep track of the scanning errors I've seen
map<uint256, int> mapSeenAlphanodeScanningErrors;
...
CAlphanode::CAlphanode(const CAlphanode& other)
{
    LOCK(cs);
    vin = other.vin;
    addr = other.addr;
...
CAlphanode::CAlphanode(CService newAddr, CTxIn newVin, CPubKey newPubkey, std::vector<unsigned char> newSig, int64_t newSigTime, CPubKey newPubkey2, int protocolVersionIn, CScript newDonationAddress, int newDonationPercentage)
{
    LOCK(cs);
    vin = newVin;
    addr = newAddr;
