...
#define ALPHANODE_ACCEPTED                    1
#define ALPHANODE_REJECTED                    0
#define ALPHANODE_RESET                       -1
...
extern std::string strAlphaNodePrivKey;
extern map<uint256, CDarksendBroadcastTx> mapDarksendBroadcastTxes;
extern CActiveAlphanode activeAlphanode;

/** Holds an AlphaSend input
 */
...
public:
    ...
    /// Add entries to use for AlphaSend
...
/**
 * A currently inprogress AlphaSend merge and denomination information
 */
class CDarksendQueue
{
public:
    ...
    /** Sign this AlphaSend transaction
     *  \return true if all conditions are met:
     *     1) we have an active Alpha Node,
     *     2) we have a valid Alpha Node private key,
    ...
    /// Is this AlphaSend expired?
    bool IsExpired()
    {
        return (GetTime() - time) > DARKSEND_QUEUE_TIMEOUT;// 120 seconds
    }

    /// Check if we have a valid Alpha Node address
    bool CheckSignature();

};

/** Helper class to store AlphaSend transaction (tx) information.
 */
class CDarksendBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 TAO - checking if valid alphanode)
...
/** Used to keep track of current status of AlphaSend pool
 */
class CDarksendPool
{
private:
    ...
    std::vector<CDarkSendEntry> entries; // Alpha Node entries
    ...
    bool sessionFoundAlphanode; //If we've found a compatible Alpha Node
...
public:
    ...
    CAlphanode* pSubmittedToAlphanode;
    ...
    CDarksendPool()
    {
        /* AlphaSend uses collateral addresses to trust parties entering the pool
    ...
    /** Process a AlphaSend message using the AlphaSend protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | AlphaSend Acceptable
     *        dsc      | AlphaSend Complete
     *        dsf      | AlphaSend Final tx
     *        dsi      | AlphaSend vIn
     *        dsq      | AlphaSend Queue
     *        dss      | AlphaSend Signal Final Tx
     *        dssu     | AlphaSend status update
     *        dssub    | AlphaSend Subscribe To
     * \param vRecv
     */
    ...
    {
        if (fAlphaNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("darksend", "CDarksendPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Alpha Node. \n");
            return;
        }
    ...
    /// Passively run AlphaSend in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false);
    bool PrepareDarksendDenominate();

    /// from alphanode-sync.h
    bool IsBlockchainSynced();

    /// Check for process in AlphaSend
    void Check();
    ...
    /// As a client, send a transaction to a Alpha Node to start the denomination process
    void SendDarksendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get Alpha Node updates about the progress of AlphaSend
