...
//step 2.) Top 10 alphanodes, open connect to top 1 alphanode. Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 alphanode, waits for 10 messages. Upon success, sends "txlock'

void ProcessMessageInstantX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if(fLiteMode) return; //disable all darksend/alphanode related functionality
    ...
    else if (strCommand == "txlvote") //InstantX Lock Consensus Votes
    {
        ...
        if(ProcessConsensusVote(pfrom, ctx)){
            //Spam/Dos protection
            /*
                Alpha Nodes will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                if(!mapUnknownVotes.count(ctx.vinAlphanode.prevout.hash)){
                    mapUnknownVotes[ctx.vinAlphanode.prevout.hash] = GetTime()+(60*10);
                }

                if(mapUnknownVotes[ctx.vinAlphanode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinAlphanode.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstantX::txlreq - alphanode is spamming transaction votes: %s %s\n",
                            ctx.vinAlphanode.ToString().c_str(),
...
bool IsIXTXValid(const CTransaction& txCollateral){
    ...
    if(nValueOut > Params().AlphaNodeCollateral()){
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which alphanodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetAlphanodeRank(activeAlphanode.vin, nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    if(n == -1)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Unknown Alpha Node\n");
        return;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Alpha Node not in the top %d (%d)\n", INSTANTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinAlphanode = activeAlphanode.vin;
...
{
    int n = mnodeman.GetAlphanodeRank(ctx.vinAlphanode, ctx.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    CAlphanode* pmn = mnodeman.Find(ctx.vinAlphanode);
    if(pmn != NULL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Alpha Node ADDR %s %d\n", pmn->addr.ToString().c_str(), n);
    }

    if(n == -1)
    {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Unknown Alpha Node\n");
        mnodeman.AskForMN(pnode, ctx.vinAlphanode);
        return false;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Alpha Node not in the top %d (%d) - %s\n", INSTANTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if(!ctx.SignatureValid()) {
        LogPrintf("InstantX::ProcessConsensusVote - Signature invalid\n");
        //don't ban, it could just be a non-synced alphanode
        mnodeman.AskForMN(pnode, ctx.vinAlphanode);
        return false;
    }
...
{
    return vinAlphanode.prevout.hash + vinAlphanode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    CAlphanode* pmn = mnodeman.Find(vinAlphanode);

    if(pmn == NULL)
    {
        LogPrintf("InstantX::CConsensusVote::SignatureValid() - Unknown Alpha Node\n");
        return false;
    }
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strAlphaNodePrivKey.c_str());

    if(!darkSendSigner.SetKey(strAlphaNodePrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid alphanodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }
...
bool CTransactionLock::SignaturesValid()
{
    ...
    {
        int n = mnodeman.GetAlphanodeRank(vote.vinAlphanode, vote.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

        if(n == -1)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Alpha Node\n");
            return false;
        }

        if(n > INSTANTX_SIGNATURES_TOTAL)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Alpha Node not in the top %s\n", INSTANTX_SIGNATURES_TOTAL);
            return false;
        }
