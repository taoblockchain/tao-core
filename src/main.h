...
#define START_ALPHANODE_PAYMENTS_TESTNET 1477958399        // Oct 31, 2016 23:59:59 GMT
#define START_ALPHANODE_PAYMENTS 1477958399
...
static unsigned int maxStandardTxSigops = maxBlockSize/5;
/** Maxiumum number of signature check operations in an IsStandard() P2SH script */
static const unsigned int MAX_P2SH_SIGOPS = 15;
/** The maximum number of orphan transactions kept in memory */
static const unsigned int MAX_ORPHAN_TRANSACTIONS = maxBlockSize/100;
...
static const int64_t MIN_INTELLITX_FEE = 10*COIN; // 10 TAO
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
static const int64_t MIN_RELAY_TX_FEE = 0.00001*COIN;
/** No amount larger than this (in satoshi) is valid */
static const int64_t MAX_MONEY = 4000000000u * COIN;
...
{
public:
    static const int CURRENT_VERSION=INTELLITX_VERSION;
    int nVersion;
    unsigned int nTime;
    ...
    bool IsIntelliTx() const 
    {
        ...
        BOOST_FOREACH(const CTxOut& out, vout)
        {
            ...
            if ((op == OP_IDENTITY      && vvch.size() == 1)
                || (op == OP_ORDER      && vvch.size() == 1)
                || (op == OP_ESCROW     && vvch.size() == 1)
                || (op == OP_ACCEPT     && vvch.size() == 1)
                || (op == OP_EXPIRE     && vvch.size() == 1)
                || (op == OP_PROVENANCE && vvch.size() == 1)
                || (op == OP_EXECUTE    && vvch.size() == 1)
                || (op == OP_UTXO       && vvch.size() == 1)
                || (op == OP_RELEASE    && vvch.size() == 1))
            {
                test = true;
            } else {
                return false;
        ...
        return (test && (vout.size() == 1));   // IntelliTx only have one vout
    }
    ...
    std::string ToString() const
    {
        ...
        str += strprintf("(hash=%s, nTime=%d, ver=%d, vin.size=%u, vout.size=%u, nLockTime=%d, data.size=%" PRIszu")\n",
            GetHash().ToString(),
            nTime,
            nVersion,
            vin.size(),
